...
<p align="center">
  ...
  <a href="#टूल-संदर्भ-55-टूल">टूल (55)</a> &bull;
...
<td>OWASP MCP Top 10 मैपिंग &mdash; 10 जोखिम श्रेणियों में 55 जांचें</td>
</tr>
<tr>
<td><b>रिपोर्ट</b></td>
...
mcp-security-scanner को **शून्य कॉन्फ़िगरेशन** चाहिए। कोई API कुंजी नहीं, कोई टोकन नहीं, कोई बाहरी सेवा नहीं। सभी 55 टूल पूरी तरह से आपकी स्थानीय मशीन पर चलते हैं।
...
<details open>
<summary><b>रनटाइम निरीक्षण (23) &mdash; कोई API कुंजी नहीं</b></summary>
...
| `rt_check_oauth` | जांचें कि HTTP/SSE सर्वर OAuth टोकन मान्य करता है या नहीं &mdash; कोई टोकन नहीं, अमान्य टोकन, और जाली JWT (alg:none) भेजता है |
| `rt_check_tls` | TLS प्रमाणपत्र का निरीक्षण करें &mdash; समय सीमा समाप्त, स्व-हस्ताक्षरित, कमजोर हस्ताक्षर (SHA-1), छोटी कुंजी (<2048 बिट), सादा HTTP |
| `rt_check_capabilities` | सर्वर क्षमताओं का निरीक्षण करें &mdash; प्रयोगात्मक सुविधाएं, गतिशील टूल परिवर्तन (listChanged), लॉगिंग, सैंपलिंग |
| `rt_check_resource_content` | readResource() के माध्यम से वास्तविक रिसोर्स सामग्री पढ़ें और पॉइज़निंग, ANSI, यूनिकोड स्टेगनोग्राफी, अत्यधिक बड़ी सामग्री के लिए स्कैन करें |
| `rt_fuzz_tools` | एज-केस इनपुट के साथ टूल का फ़ज़-टेस्ट करें &mdash; पथ ट्रैवर्सल, कमांड इंजेक्शन, SQL इंजेक्शन, टाइप कन्फ्यूज़न (डिफ़ॉल्ट रूप से ड्राई-रन) |
| `rt_check_http_security` | HTTP रिस्पॉन्स हेडर जांचें &mdash; HSTS, CORS, X-Content-Type-Options, Cache-Control, कुकी फ्लैग |
| `rt_check_callbacks` | कॉलबैक/वेबहुक URL पैरामीटर पहचानें जो SSRF सक्षम कर सकते हैं &mdash; अनुपस्थित URL प्रतिबंधों की जांच करता है |
| `rt_check_prompt_injection` | getPrompt() के माध्यम से प्रॉम्प्ट सामग्री प्राप्त करें और इंजेक्शन पैटर्न, टेम्पलेट सिंटैक्स, खतरनाक आर्गुमेंट के लिए स्कैन करें |
| `rt_check_instructions` | पॉइज़निंग, सोशल इंजीनियरिंग, अत्यधिक लंबाई के लिए इनिशियलाइज़ेशन से सर्वर निर्देशों का विश्लेषण करें |
| `rt_check_tool_mutation` | कॉन्फ़िगर करने योग्य विलंब के साथ दोहरे-स्नैपशॉट तुलना &mdash; टूल जोड़ना, हटाना, विवरण परिवर्तन (रग पुल) पहचानें |
| `rt_check_rate_limiting` | रेट लिमिटिंग परीक्षण के लिए तेज़ ping() बर्स्ट भेजें &mdash; असीमित अनुरोध स्वीकार करने वाले सर्वरों को फ्लैग करता है |
| `rt_check_protocol_version` | इनिशियलाइज़ेशन से सर्वर नाम/संस्करण जांचें &mdash; अनुपस्थित जानकारी, पुराने SDK संस्करणों को फ्लैग करता है |
...
| `report_full_audit` | सभी 55 जांचें चलाएं और OWASP स्कोरिंग के साथ एक व्यापक सुरक्षा ऑडिट रिपोर्ट जनरेट करें |
...
| टूल | विवरण |
|------|-------|
| `scanner_list_checks` | श्रेणियों, गंभीरता स्तरों, और OWASP MCP Top 10 मैपिंग के साथ सभी 55 सुरक्षा जांचें सूचीबद्ध करें |
...
mcp-security-scanner सभी 55 जांचों को [OWASP MCP Top 10](https://owasp.org/www-project-model-context-protocol-top-10/) जोखिम फ्रेमवर्क से मैप करता है।

| ID | जोखिम | स्कैनर जांचें |
|----|-------|--------------|
| **MCP01** | टूल पॉइज़निंग | `rt_check_scope_creep`, `rt_check_capabilities`, `cfg_check_context_oversharing` |
| **MCP02** | अत्यधिक अनुमतियां | `rt_check_scope_creep`, `rt_check_resource_exposure`, `rt_check_callbacks`, `cfg_check_context_oversharing` |
| **MCP03** | टूल शैडोइंग | `rt_check_tool_poisoning`, `rt_check_ansi_injection`, `rt_check_unicode_steganography`, `rt_check_resource_content`, `rt_check_prompt_injection`, `rt_check_instructions` |
| **MCP04** | असुरक्षित क्रेडेंशियल संग्रहण | `dep_audit_lockfile`, `dep_check_typosquatting`, `dep_check_install_scripts`, `dep_check_unpinned`, `dep_check_license`, `dep_check_mcp_sdk_version` |
| **MCP05** | डेटा लीकेज | `sast_command_injection`, `sast_ssrf`, `sast_path_traversal`, `sast_code_execution`, `sast_prototype_pollution`, `rt_fuzz_tools` |
| **MCP06** | कोड इंजेक्शन | `rt_check_tool_shadowing`, `rt_check_cross_origin`, `rt_check_tool_mutation`, `rt_check_capabilities` |
| **MCP07** | तृतीय-पक्ष / सप्लाई चेन जोखिम | `rt_check_auth`, `rt_check_oauth`, `rt_check_tls`, `rt_check_http_security`, `rt_check_protocol_version`, `cfg_check_transport_security` |
| **MCP08** | अपर्याप्त लॉगिंग | `sast_missing_logging`, `rt_check_rate_limiting`, `rt_fuzz_tools` |
| **MCP09** | रग पुल / टूल संशोधन | `rt_pin_tools`, `rt_verify_pins`, `rt_check_tool_mutation`, `cfg_check_shadow_servers`, `report_compare` |
| **MCP10** | सर्वर गलत कॉन्फ़िगरेशन | `rt_check_resource_exposure`, `rt_check_resource_content`, `sast_info_disclosure`, `cfg_check_context_oversharing`, `sast_hardcoded_secrets`, `cfg_scan_env_files` |
...
# सुविधा कमांड
mcp-security-scanner --full-audit .           # पूर्ण सुरक्षा ऑडिट (सभी 55 जांचें)
...
src/
  ...
  protocol/
    ...
    tools.ts                  # टूल रजिस्ट्री — सभी 55 टूल यहां एकत्रित
  types/
  ...
  runtime/                    # रनटाइम निरीक्षण टूल (23)
    ...
    oauth-checker.ts          # OAuth टोकन सत्यापन (कोई टोकन नहीं, अमान्य, जाली JWT)
    tls-checker.ts            # TLS प्रमाणपत्र निरीक्षण (समय सीमा समाप्त, स्व-हस्ताक्षरित, कमजोर)
    capabilities-checker.ts   # सर्वर क्षमताएं और प्रयोगात्मक सुविधा निरीक्षण
    resource-content-checker.ts # वास्तविक रिसोर्स सामग्री पढ़ना और स्कैन करना
    fuzzer.ts                 # एज-केस इनपुट के साथ टूल फ़ज़-टेस्ट
    http-security-checker.ts  # HTTP सुरक्षा हेडर जांच
    callback-checker.ts       # कॉलबैक/वेबहुक URL पैरामीटर पहचान
    prompt-injection-checker.ts # इंजेक्शन पैटर्न के लिए प्रॉम्प्ट सामग्री स्कैन
    instructions-checker.ts   # सर्वर निर्देश विश्लेषण
    mutation-checker.ts       # टूल परिवर्तन पहचान के लिए दोहरे-स्नैपशॉट तुलना
    rate-limit-checker.ts     # ping बर्स्ट के साथ रेट लिमिटिंग परीक्षण
    protocol-version-checker.ts # सर्वर नाम/संस्करण जांच
...
<td>~10</td>
<td>~5</td>
<td><b>6 श्रेणियों में 55 टूल</b></td>
</tr>
</tbody>
</table>
...
| **mcp-security-scanner** | **MCP सर्वर सुरक्षा स्कैनिंग** | **55 टूल, 6 श्रेणियां** |
