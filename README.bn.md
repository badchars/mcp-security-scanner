...
<p align="center">
  ...
  <a href="#টুল-রেফারেন্স-৫৫-টুল">টুল (৫৫)</a> &bull;
...
**mcp-security-scanner** আপনার AI এজেন্টকে ৬টি বিভাগে ৫৫টি টুল দেয়। এজেন্ট যেকোনও MCP সার্ভারের সাথে সংযুক্ত হয়, লাইভ টুল পরিদর্শন করে, AST-ভিত্তিক স্ট্যাটিক বিশ্লেষণ সহ সোর্স কোড স্কান করে, কনফিগ অডিট করে, নির্ভরতা পরীক্ষা করে এবং OWASP MCP Top 10 সম্মতি স্কোর সহ রিপোর্ট তৈরি করে &mdash; সবকিছু একটি একক কথোপকথনে।
...
<td>OWASP MCP Top 10 ম্যাপিং &mdash; ১০টি ঝুঁকি বিভাগ জুড়ে ৫৫টি চেক</td>
</tr>
<tr>
<td><b>রিপোর্ট</b></td>
...
mcp-security-scanner **শূন্য কনফিগারেশন** প্রয়োজন। কোনও API কী নেই, কোনও টোকেন নেই, কোনও বাহ্যিক সেবা নেই। সমস্ত ৫৫টি টুল সম্পূর্ণরূপে আপনার স্থানীয় মেশিনে চলে।
...
## টুল রেফারেন্স (৫৫ টুল)

<details open>
<summary><b>রানটাইম পরিদর্শন (২৩) &mdash; কোনও API কী নেই</b></summary>
...
| `rt_check_oauth` | HTTP/SSE সার্ভার OAuth টোকেন যাচাই করে কিনা পরীক্ষা করুন &mdash; কোনো টোকেন নেই, অবৈধ টোকেন এবং জাল JWT (alg:none) পাঠায় |
| `rt_check_tls` | TLS সার্টিফিকেট পরিদর্শন করুন &mdash; মেয়াদোত্তীর্ণ, স্ব-স্বাক্ষরিত, দুর্বল স্বাক্ষর (SHA-1), ছোট কী (<2048 বিট), সাধারণ HTTP |
| `rt_check_capabilities` | সার্ভার ক্ষমতা পরিদর্শন করুন &mdash; পরীক্ষামূলক ফিচার, গতিশীল টুল পরিবর্তন (listChanged), লগিং, স্যাম্পলিং |
| `rt_check_resource_content` | readResource() এর মাধ্যমে প্রকৃত রিসোর্স কন্টেন্ট পড়ুন এবং বিষক্রিয়া, ANSI, Unicode স্টেগানোগ্রাফি, অতিরিক্ত বড় কন্টেন্টের জন্য স্ক্যান করুন |
| `rt_fuzz_tools` | এজ-কেস ইনপুট দিয়ে টুল ফাজ-টেস্ট করুন &mdash; পথ ট্রাভার্সাল, কমান্ড ইনজেকশন, SQL ইনজেকশন, টাইপ কনফিউশন (ডিফল্টে ড্রাই-রান) |
| `rt_check_http_security` | HTTP রেসপন্স হেডার পরীক্ষা করুন &mdash; HSTS, CORS, X-Content-Type-Options, Cache-Control, কুকি ফ্ল্যাগ |
| `rt_check_callbacks` | SSRF সক্রিয় করতে পারে এমন callback/webhook URL প্যারামিটার সনাক্ত করুন &mdash; অনুপস্থিত URL সীমাবদ্ধতা পরীক্ষা করে |
| `rt_check_prompt_injection` | getPrompt() এর মাধ্যমে প্রম্পট কন্টেন্ট আনুন এবং ইনজেকশন প্যাটার্ন, টেমপ্লেট সিনট্যাক্স, বিপজ্জনক আর্গুমেন্টের জন্য স্ক্যান করুন |
| `rt_check_instructions` | বিষক্রিয়া, সোশ্যাল ইঞ্জিনিয়ারিং, অতিরিক্ত দৈর্ঘ্যের জন্য ইনিশিয়ালাইজেশন থেকে সার্ভার নির্দেশাবলী বিশ্লেষণ করুন |
| `rt_check_tool_mutation` | কনফিগারযোগ্য বিলম্ব সহ দ্বৈত-স্ন্যাপশট তুলনা &mdash; টুল সংযোজন, অপসারণ, বিবরণ পরিবর্তন সনাক্ত করুন (rug pull) |
| `rt_check_rate_limiting` | রেট লিমিটিং পরীক্ষা করতে দ্রুত ping() বার্স্ট পাঠান &mdash; সীমাহীন অনুরোধ গ্রহণকারী সার্ভারগুলির জন্য সতর্ক করে |
| `rt_check_protocol_version` | ইনিশিয়ালাইজেশন থেকে সার্ভারের নাম/সংস্করণ পরীক্ষা করুন &mdash; অনুপস্থিত তথ্য, পুরানো SDK সংস্করণের জন্য সতর্ক করে |
...
| `report_full_audit` | সমস্ত ৫৫টি চেক চালান এবং OWASP স্কোরিং সহ একটি বিস্তৃত নিরাপত্তা অডিট রিপোর্ট তৈরি করুন |
...
| টুল | বিবরণ |
|------|-------------|
| `scanner_list_checks` | বিভাগ, গুরুত্ব স্তর এবং OWASP MCP Top 10 ম্যাপিং সহ সমস্ত ৫৫টি নিরাপত্তা চেক তালিকা করুন |
...
mcp-security-scanner সমস্ত ৫৫টি চেক [OWASP MCP Top 10](https://owasp.org/www-project-model-context-protocol-top-10/) ঝুঁকি কাঠামোতে ম্যাপ করে।

| ID | ঝুঁকি | স্কানার চেক |
|----|------|----------------|
| **MCP01** | টুল বিষক্রিয়া | `rt_check_scope_creep`, `rt_check_capabilities`, `cfg_check_context_oversharing` |
| **MCP02** | অতিরিক্ত অনুমতি | `rt_check_scope_creep`, `rt_check_resource_exposure`, `rt_check_callbacks`, `cfg_check_context_oversharing` |
| **MCP03** | টুল ছায়া | `rt_check_tool_poisoning`, `rt_check_ansi_injection`, `rt_check_unicode_steganography`, `rt_check_resource_content`, `rt_check_prompt_injection`, `rt_check_instructions` |
| **MCP04** | অনিরাপদ শংসাপত্র সংরক্ষণ | `dep_audit_lockfile`, `dep_check_typosquatting`, `dep_check_install_scripts`, `dep_check_unpinned`, `dep_check_license`, `dep_check_mcp_sdk_version` |
| **MCP05** | ডেটা লিকেজ | `sast_command_injection`, `sast_ssrf`, `sast_path_traversal`, `sast_code_execution`, `sast_prototype_pollution`, `rt_fuzz_tools` |
| **MCP06** | কোড ইনজেকশন | `rt_check_tool_shadowing`, `rt_check_cross_origin`, `rt_check_tool_mutation`, `rt_check_capabilities` |
| **MCP07** | তৃতীয় পক্ষ / সাপ্লাই চেইন ঝুঁকি | `rt_check_auth`, `rt_check_oauth`, `rt_check_tls`, `rt_check_http_security`, `rt_check_protocol_version`, `cfg_check_transport_security` |
| **MCP08** | অপর্যাপ্ত লগিং | `sast_missing_logging`, `rt_check_rate_limiting`, `rt_fuzz_tools` |
| **MCP09** | Rug Pull / টুল সংশোধন | `rt_pin_tools`, `rt_verify_pins`, `rt_check_tool_mutation`, `cfg_check_shadow_servers`, `report_compare` |
| **MCP10** | সার্ভার ভুল কনফিগারেশন | `rt_check_resource_exposure`, `rt_check_resource_content`, `sast_info_disclosure`, `cfg_check_context_oversharing`, `sast_hardcoded_secrets`, `cfg_scan_env_files` |
...
# সুবিধা কমান্ড
mcp-security-scanner --full-audit .           # সম্পূর্ণ নিরাপত্তা অডিট (সমস্ত ৫৫টি চেক)
...
src/
  ...
  protocol/
    ...
    tools.ts                  # টুল রেজিস্ট্রি — সমস্ত ৫৫টি টুল এখানে একত্রিত
  types/
  ...
  runtime/                    # রানটাইম পরিদর্শন টুল (২৩)
    ...
    oauth-checker.ts          # OAuth টোকেন যাচাই (কোনো টোকেন নেই, অবৈধ, জাল JWT)
    tls-checker.ts            # TLS সার্টিফিকেট পরিদর্শন (মেয়াদোত্তীর্ণ, স্ব-স্বাক্ষরিত, দুর্বল)
    capabilities-checker.ts   # সার্ভার ক্ষমতা এবং পরীক্ষামূলক ফিচার পরিদর্শন
    resource-content-checker.ts # প্রকৃত রিসোর্স কন্টেন্ট পড়া এবং স্ক্যান করা
    fuzzer.ts                 # এজ-কেস ইনপুট দিয়ে টুল ফাজ-টেস্ট
    http-security-checker.ts  # HTTP নিরাপত্তা হেডার পরীক্ষা
    callback-checker.ts       # callback/webhook URL প্যারামিটার সনাক্তকরণ
    prompt-injection-checker.ts # ইনজেকশন প্যাটার্নের জন্য প্রম্পট কন্টেন্ট স্ক্যান
    instructions-checker.ts   # সার্ভার নির্দেশাবলী বিশ্লেষণ
    mutation-checker.ts       # টুল পরিবর্তন সনাক্তকরণের জন্য দ্বৈত-স্ন্যাপশট তুলনা
    rate-limit-checker.ts     # ping বার্স্ট দিয়ে রেট লিমিটিং পরীক্ষা
    protocol-version-checker.ts # সার্ভারের নাম/সংস্করণ পরীক্ষা
...
<td>~১০</td>
<td>~৫</td>
<td><b>৬টি বিভাগ জুড়ে ৫৫টি টুল</b></td>
</tr>
</tbody>
</table>
...
| **mcp-security-scanner** | **MCP সার্ভার নিরাপত্তা স্কানিং** | **৫৫ টুল, ৬ বিভাগ** |
